...
trait Menu
{
    ...
    {
        if (!isset($cutomeIcon)) {
            return '<i class="fa-solid fa-circle" style="font-size: .625rem"></i>';
        }

        $attrs = isset($title) 
            ? sprintf(
                'data-sidebar-tooltip="tooltip" data-bs-placement="right" data-bs-title="%s" aria-label="%s"',
                htmlspecialchars($title, ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($title, ENT_QUOTES, 'UTF-8')
            )
            : '';

        return "<i class='icon $cutomeIcon' $attrs></i>";
    }
}

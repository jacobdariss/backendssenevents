        ...
        if (in_array($extension, ['csv'])) {
            ...
            try {
                ...
                $rows = collect($this->processCsvFile($fullPath, [
                    'Name', 'Description', 'TV Show', 'Season Index', 'Status', 'Trailer URL Type', 
                    'Trailer URL', 'Poster URL','Poster TV URL', 'Price', 'Purchase Type',
        ...
        foreach ($rows->chunk($this->batchSize) as $batchIndex => $batch) {
            ...
            foreach ($batch as $rowIndex => $row) {
                ...
                try {
                    ...
                    // Inherit access/plan from the TV show so seasons
                    // follow the same access level and subscription plan.
                    $seasonAccess = $tvShow->movie_access ?? null;
                    $seasonPlanId = $tvShow->plan_id ?? null;

                    // Fallbacks in case TV show has no access/plan defined
                    if (empty($seasonAccess)) {
                        $seasonAccess = 'free';
                    }

                    // Determine correct status value
                    $statusValue = $data['Status'] ?? '';
                   ...
                   if (!empty($statusValue) && in_array(strtolower($statusValue), ['active', 'inactive', '1', '0'])) {
                    ...
                    $seasonData = [
                        ...
                        // inherit from TV show
                        'access' => $seasonAccess,
                        'plan_id' => $seasonPlanId,

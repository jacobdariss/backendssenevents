    ...
    @if (!$data->trashed())
        @hasPermission('edit_tvcategory')
            <a class="btn btn-warning-subtle btn-sm fs-4" data-bs-toggle="tooltip" title="{{ __('messages.edit') }}"
                href="{{ route('backend.tv-category.edit', $data->id) }}"> <i
                    class="ph ph-pencil-simple-line align-middle"></i></a>
        @endhasPermission
        @hasPermission('delete_tvcategory')
            <a href="{{ route('backend.tv-category.destroy', $data->id) }}" id="delete-locations-{{ $data->id }}"
    ...
    @else
        @hasPermission('restore_tvcategory')
            <a class="btn btn-success-subtle btn-sm fs-4 restore-tax"
                ...
                data-success-message="{{ __('messages.restore_form', ['form' => 'Tv Category']) }}"
                href="{{ route('backend.tv-category.restore', $data->id) }}">
                <i class="ph ph-arrow-clockwise align-middle"></i>
            </a>
        @endhasPermission

        @haspermission('force_delete_tvcategory')
            <a href="{{ route('backend.tv-category.force_delete', $data->id) }}"
                ...
                title="{{ __('messages.force_delete') }}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_tv_category', ['name' => $data->name]) }}">
                <i class="ph ph-trash align-middle"></i>
            </a>
        @endhasPermission
    @endif

<div class="d-flex gap-2 align-items-center justify-content-end">
  ...
  @else
      ...
      data-success-message="{{__('messages.restore_form',  ['form' => 'Tv Channel'])}}" href="{{ route('backend.tv-channel.restore', $data->id) }}">
        ...
        <!-- Force Delete link -->
        <a href="{{ route('backend.tv-channel.force_delete', $data->id) }}" id="delete-locations-{{ $data->id }}" class="btn btn-danger-subtle btn-sm fs-4" data-type="ajax" data-method="DELETE" data-token="{{ csrf_token() }}" data-bs-toggle="tooltip" title="{{ __('messages.force_delete') }}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_tv_channel', ['name' => $data->name]) }}">
            <i class="ph ph-trash align-middle"></i>
        </a>

<div class="d-flex gap-2 align-items-center justify-content-end">
   ...
   data-success-message="{{__('messages.restore_form_video',  ['form' => 'Video'])}}" data-bs-toggle="tooltip" title="{{__('messages.restore')}}" href="{{ route('backend.videos.restore', $data->id) }}">
        ...
        <a href="{{ route('backend.videos.force_delete', $data->id) }}"  data-bs-toggle="tooltip"
            title="{{ __('messages.force_delete') }}" id="delete-{{ $module_name }}-{{ $data->id }}" class="btn btn-danger-subtle btn-sm fs-4" data-type="ajax" data-method="DELETE" data-token="{{ csrf_token() }}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_video', ['name' => $data->name]) }}">
            <i class="ph ph-trash align-middle"></i>
        </a>
        @endhasPermission
    @endif

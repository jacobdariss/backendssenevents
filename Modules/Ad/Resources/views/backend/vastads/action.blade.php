<div class="d-flex gap-2 align-items-center justify-content-end">
    ...
    data-success-message="{{trans('messages.restore_form')}}" href="{{ route('backend.vastads.restore', $data->id) }}" data-bs-toggle="tooltip" title="{{ __('messages.restore') }}">
        ...
        <!-- Force Delete link -->
        <a href="{{ route('backend.vastads.force_delete', $data->id) }}" id="delete-vastads-{{ $data->id }}" class="btn btn-danger-subtle btn-sm fs-4" data-type="ajax" data-method="DELETE" data-token="{{ csrf_token() }}" data-bs-toggle="tooltip" title="{{ __('messages.force_delete') }}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_vastads', ['name' => $data->name]) }}">
            <i class="ph ph-trash align-middle"></i>
        </a>
        @endhasPermission
    @endif

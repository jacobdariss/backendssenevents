<div class="d-flex gap-2 align-items-center justify-content-end">
    ...
    data-success-message="{{__('messages.restore_form',  ['form' => 'Genres'])}}" data-bs-toggle="tooltip" title="{{__('messages.restore')}}" href="{{ route('backend.genres.restore', $data->id) }}">
        ...
        <a href="{{route('backend.genres.force_delete', $data->id)}}" id="delete-locations-{{$data->id}}" class="btn btn-danger-subtle btn-sm fs-4" data-type="ajax" data-method="DELETE" data-token="{{csrf_token()}}" data-bs-toggle="tooltip" title="{{__('messages.force_delete')}}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_entity', ['name' => $data->name, 'entity' => 'Genre']) }}"> <i class="ph ph-trash align-middle"></i></a>
        @endhasPermission
    @endif
</div>

                ...
                <div class="movie-detail-content">
                    ...
                    @if ($data['movie_access'] == 'pay-per-view')
                        <div class="bg-dark text-white p-3 my-2 rounded d-flex flex-wrap justify-content-between align-items-center gap-3"
                            ...
                            <div>
                                @php
                                    $rentalStartDuration = trans_choice(
                                        'messages.lbl_day_text',
                                        $data['available_for'],
                                        ['count' => $data['available_for']],
                                    );
                                    $rentalWatchDuration = trans_choice(
                                        'messages.lbl_day_text',
                                        $data['access_duration'],
                                        ['count' => $data['access_duration']],
                                    );
                                @endphp
                                    ...
                                    <div>
                                        @if ($data['purchase_type'] === 'rental')
                                            <a href="{{ route('pay-per-view.paymentform', ['id' => $data['id']]) }}"
                                                ...
                                                @if ($data['discount'] > 0)
                                                    <span class="me-2">
                                                        {!! __('messages.rent_button', [
                                                            'price' =>
                                                                '<span dir="ltr">' .
                                                                Currency::format($data['price'] - $data['price'] * ($data['discount'] / 100), 2) .
                                                                '</span>',
                                                        ]) !!}
                                                    </span>
                                                    <span class="text-decoration-line-through text-white-50">
                                                        <span
                                                            dir="ltr">{{ Currency::format($data['price'], 2) }}</span>
                                                    </span>
                                                @else
                                                    <span>{!! __('messages.rent_button', [
                                        ...
                                        @else
                                            <a href="{{ route('pay-per-view.paymentform', ['id' => $data['id']]) }}"
                                                ...
                                                @else
                                                    <span>{!! __('messages.one_time_button', [
                                                        'price' => '<span dir="ltr">' . Currency::format($data['price'], 2) . '</span>',
                                                    ]) !!}</span>
                                                @endif
                                            </a>
                    ...
                    <p class="font-size-14 js-episode-desc">
                        ...
                        @if (strlen(strip_tags($data['description'])) > 300)
                            <a href="javascript:void(0)"
                                class="btn btn-link p-0 align-baseline js-episode-toggle">{{ __('messages.read_more') }}</a>
                        @endif
                    </p>
                        ...
                        <li>
                            @if ($data['imdb_rating'])
                                <span class="d-flex align-items-center gap-2">
                                    ...
                                    <span class="fw-medium">{{ $data['imdb_rating'] }}
                                        ({{ __('messages.lbl_IMDb') }})</span>
                                </span>
                            @endif
                        </li>
                    </ul>
                    @php
                        ...
                        if ($data['type'] == 'movie') {
                            ...
                            if (
                                ...
                                !empty($data['bunny_video_url']) &&
                                config('filesystems.active') == 'bunny'
                            ) {
                                $type = 'HLS';
                            ...
                            if (
                                $episodeData['video_upload_type'] == 'Local' &&
                                !empty($episodeData['bunny_video_url'] && config('filesystems.active') == 'bunny')
                            ) {
                                $type = 'HLS';
                    ...
                    @if (
                        $data['movie_access'] == 'pay-per-view' &&
                            \Modules\Entertainment\Models\Entertainment::isPurchased($data['id'], $data['type']) === false)
        ...
        <div class="modal-content section-bg text-white rounded shadow border-0 p-4">
            ...
            <div class="rounded p-3 p-lg-5 mb-4 bg-dark">
                ...
                <ul class="font-size-14 text-body">
                    <li>{!! __('messages.info_start_days', [
                        'duration' => trans_choice('messages.lbl_day_text', $data['available_for'], ['count' => $data['available_for']]),
                    ]) !!}</li>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm" style="max-width:500px;">
        <div class="modal-content section-bg text-white rounded shadow-lg border-0 p-4">
            ...
            <div class="rounded p-3 p-lg-5 mb-4 bg-dark">
                ...
                <ul class="font-size-14 text-body ">
                    <li>{!! __('messages.rental_info_start', [
                        'duration' => trans_choice('messages.lbl_day_text', $data['available_for'], ['count' => $data['available_for']]),
                    ]) !!}</li>
                    <li>{!! __('messages.rental_info_duration', [
                        'duration' => trans_choice('messages.lbl_day_text', $data['access_duration'], [
                            'count' => $data['access_duration'],
                        ]),
                    ]) !!}</li>
                    <li>{!! __('messages.info_non_refundable') !!}</li>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('copyLink')?.addEventListener('click', function(e) {
            ...
            navigator.clipboard?.writeText(this.getAttribute('data-link')).then(() => {
                ...
                setTimeout(() => document.getElementById('copyFeedback').style.display = 'none',
                    2000);
            });
        });
    });

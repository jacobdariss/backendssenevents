<div class="detail-page-banner">
    <div class="video-player-wrapper" id="vp-wrapper">
        ...
        {{-- CSS chargés immédiatement (légers, pas bloquants) --}}
        <link rel="stylesheet" href="{{ asset('css/video-js.css') }}">
        <link rel="stylesheet" href="{{ asset('css/videojs.ima.css') }}">

        {{-- ── Poster overlay (masqué au clic, zéro impact perf) ── --}}
        <div id="vp-poster"
             style="position:relative;width:100%;background:#000;cursor:pointer;
                    aspect-ratio:16/9;overflow:hidden;z-index:2;">
            <img src="{{ $thumbnail_image }}" alt="poster"
        ...
        {{-- ── Player Video.js (visible après clic sur poster) ── --}}
...
{{-- Scripts Video.js — chargés normalement (requis par videoplayer.min.js) --}}
...
(function () {
    ...
    function showPlayer() {
        ...
        if (badge) {
            ...
            if (vid) vid.addEventListener('volumechange', function () {
                if (!vid.muted) { badge.style.opacity = '0'; setTimeout(function(){badge.style.display='none';},300); }
            }, { once: true });
        }
    }

    // Clic sur le poster → afficher le player
    if (poster) poster.addEventListener('click', showPlayer);

    // Clic sur "Regarder maintenant" → afficher le player
    document.addEventListener('click', function (e) {
        var btn = e.target && (e.target.id === 'watchNowButton' ? e.target : e.target.closest('#watchNowButton'));
        if (btn) showPlayer();
    });

    // Si continue_watch=true → afficher directement sans attendre le clic
    ...
    if (btn && poster) {
        poster.addEventListener('mouseenter', function(){ btn.style.background='rgba(255,255,255,.28)'; btn.style.transform='translate(-50%,-50%) scale(1.08)'; });
        poster.addEventListener('mouseleave', function(){ btn.style.background='rgba(255,255,255,.15)'; btn.style.transform='translate(-50%,-50%) scale(1)'; });
    }
})();
</script>

<style>
@keyframes vp-fadein { from{opacity:0;transform:translateY(-4px)} to{opacity:1;transform:none} }
#vp-muted-badge { animation: vp-fadein .3s ease; }

.video-player-wrapper { position: relative; }
#vimeoContainer { position:relative;padding-bottom:56.25%;height:0;overflow:hidden;display:none; }
#vimeoIframe    { position:absolute;top:0;left:0;width:100%;height:100%;display:none; }

#customAdModal  {
    display:none;position:absolute;z-index:9999;top:0;left:0;width:100%;height:100%;
...
#customAdContent {
    max-width:900px;width:90%;max-height:85vh;position:relative;background:#000;
...
#customAdCloseBtn {
    position:absolute;top:15px;right:15px;background:rgba(255,0,0,.85);color:#fff;
    border:2px solid #fff;border-radius:50%;width:36px;height:36px;font-size:22px;
    line-height:1;display:flex;align-items:center;justify-content:center;
    cursor:pointer;z-index:10001;transition:all .2s;padding:0;
}
#customAdCloseBtn:hover { background:#f00;transform:scale(1.1); }

/* IMA skip masqués */
.ima-skip-container,.ima-skip-button,div[class*="ima-ad-skip"],div[class*="ima_skip"],
div[class*="ima-skip"],button[class*="ima-skip"],button[class*="ima_skip"],
div[class*="skip-button"],.ima-skip-button-container,[class*="skip-container"],
[data-skip-button],[data-skip-container] {
...
.video-js .vjs-text-track-cue,.video-js .vjs-text-track-cue div { font-size:1.4em!important;line-height:normal!important; }
@supports (-webkit-touch-callout:none){
    .video-js .vjs-text-track-display,.video-js .vjs-text-track-cue,.video-js .vjs-text-track-cue div {
        font-size:1.4em!important;line-height:normal!important;transform:none!important;
    }
...
@media(max-width:768px){ .video-js .vjs-text-track-cue,.video-js .vjs-text-track-cue div{font-size:16px!important;} }
@media(max-width:480px){ .video-js .vjs-text-track-cue,.video-js .vjs-text-track-cue div,video::-webkit-media-text-track-display{font-size:13px!important;} }

.video-js.vjs-ima { overflow:visible; }
.video-js.vjs-ima .vjs-ima-ad-container { position:absolute;top:0;right:0;bottom:0;left:0;pointer-events:none; }
.video-js.vjs-ima .vjs-ima-ad-container>div { pointer-events:auto; }

.vjs-skip-ad-button {
    position:absolute;bottom:80px;right:20px;background:rgba(15,15,15,.85);
    backdrop-filter:blur(10px);color:#fff;padding:12px 24px;
    border:1px solid rgba(255,255,255,.2);border-radius:4px;cursor:pointer;
    z-index:10001!important;font-size:16px;font-weight:600;transition:all .2s;

...
function slickBanner(isRTL) {
  jQuery('.slick-banner').each(function () {
    ...
    bannerSlider.slick({
      ...
      autoplaySpeed: bannerSlider.data("autoplay-speed") || 3000,
...
function selectProfileSlider(isRTL) {
  const $slider = jQuery('.select-profile-slider');
  ...
  $slider.slick({
    ...
    arrows: true,
    dots: true,
    prevArrow: "<span class='slick-arrow slick-arrow-prev'><i class='ph ph-caret-left'></i></span>",
    nextArrow: "<span class='slick-arrow slick-arrow-next'><i class='ph ph-caret-right'></i></span>",
    responsive: [
      {
        breakpoint: 992,
        settings: {
  ...
  // Sync the checked radio with the currently centered slide (fix: selected image not saving)
  $slider.on('afterChange', function(event, slick, currentSlide) {
    const $centerSlide = jQuery('.select-profile-slider .slick-slide.slick-center');
    const $radio = $centerSlide.find('input[name="profile_image"]');
    if ($radio.length) {
      $radio.prop('checked', true);
    }
  });

  // On init, ensure the center slide's radio is checked (only in add mode - skip when editing)

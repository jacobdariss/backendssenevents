...
@hasPermission('restore_page')
  ...
  @hasPermission('force_delete_page')
    <!-- Force Delete link -->
  @if(!in_array($data->slug, ['privacy-policy', 'terms-conditions']))
  <a href="{{ route('backend.pages.force_delete', $data->id) }}" id="delete-{{ $module_name }}-{{ $data->id }}" class="btn btn-danger-subtle btn-sm fs-4" data-type="ajax" data-method="DELETE" data-token="{{ csrf_token() }}" data-bs-toggle="tooltip" title="{{ __('messages.force_delete') }}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_entity', ['name' => $data->name, 'entity' => 'Page']) }}">
              <i class="ph ph-trash align-middle"></i>
          </a>
  @endif
        @endhasPermission

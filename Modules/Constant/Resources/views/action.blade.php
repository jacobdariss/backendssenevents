<div class="d-flex gap-2 align-items-center justify-content-end">
   ...
   data-success-message="{{ __('messages.restore_form') }}" href="{{ route('backend.constants.restore', $data->id) }}" data-bs-toggle="tooltip" title="{{ __('messages.restore') }}">
        ...
        <!-- Force Delete link -->
         <a href="{{ route('backend.constants.force_delete', $data->id) }}" id="delete-{{ $module_name }}-{{ $data->id }}" class="btn btn-danger-subtle btn-sm fs-4" data-type="ajax" data-method="DELETE" data-token="{{ csrf_token() }}" data-bs-toggle="tooltip" title="{{ __('messages.force_delete') }}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_constant', ['name' => $data->name]) }}">
            <i class="ph ph-trash align-middle"></i>
        </a>
        @endhasPermission
    @endif

 ...
 <a class="btn btn-info-subtle btn-sm fs-4" href="{{ route('backend.seasons.details', $data->id) }}" data-bs-toggle="tooltip" title="{{ __('messages.details') }}">
  ...
  @else
   ...
   data-success-message="{{__('messages.restore_form',  ['form' => 'Season'])}}" data-bs-toggle="tooltip" title="{{__('messages.restore')}}" href="{{ route('backend.seasons.restore', $data->id) }}">
      ...
      <a href="{{ route('backend.seasons.force_delete', $data->id) }}" id="delete-locations-{{ $data->id }}" class="btn btn-danger-subtle btn-sm fs-4" data-type="ajax" data-method="DELETE" data-token="{{ csrf_token() }}" data-bs-toggle="tooltip" title="{{ __('messages.force_delete') }}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_season', ['name' => $data->name]) }}">
          <i class="ph ph-trash align-middle"></i>
      </a>
      @endhasPermission
  @endif

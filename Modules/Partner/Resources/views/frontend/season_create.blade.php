...
@section('title') {{ __('partner::partner.add_season') }} @endsection

@section('content')
<x-back-button-component :route="route('partner.tvshow.seasons', $tvshow->id)" />

{{ html()->form('POST', route('partner.tvshow.season.store', $tvshow->id))->attribute('enctype', 'multipart/form-data')->attribute('id', 'form-submit')->open() }}
...
<div class="card mb-3">
    <div class="card-header">
        <h5 class="mb-0"><i class="ph ph-plus-circle me-2"></i>{{ __('partner::partner.add_season') }} — <span class="text-muted">{{ $tvshow->name }}</span></h5>
    </div>
    <div class="card-body">
        <div class="row gy-3">
            ...
            <div class="col-md-4">
                ...
                {{ html()->number('season_number', old('season_number', 1))->class('form-control')->attribute('min', 1)->attribute('required') }}

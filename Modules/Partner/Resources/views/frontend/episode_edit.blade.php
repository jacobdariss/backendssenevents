...
@if($episode->approval_status === 'rejected' && $episode->rejection_reason)
    <div class="alert alert-danger d-flex gap-2 mb-3">
        <i class="ph ph-warning fs-5 mt-1"></i>
        <div><strong>{{ __('partner::partner.rejection_reason') }} :</strong> {{ $episode->rejection_reason }}</div>
    </div>
@endif
...
<div class="card mb-3">
    <div class="card-header">
        <h5 class="mb-0">
            <i class="ph ph-film-strip me-2"></i>{{ __('partner::partner.edit_episode') }}
    ...
    <div class="card-body">
        <div class="row gy-3">
            ...
            <div class="col-md-6 position-relative">
                ...
                <div class="uploaded-image" id="selectedImageContainerPoster">@if($episode->poster_url)<img src="{{ $episode->poster_url }}" class="img-fluid mt-1 box-preview-image">@endif</div>
            ...
            <div class="col-md-6 position-relative">
                ...
                <div class="uploaded-image" id="selectedImageContainerPosterTv">@if($episode->poster_tv_url)<img src="{{ $episode->poster_tv_url }}" class="img-fluid mt-1 box-preview-image">@endif</div>
                    ...
                    <label class="form-check form-check-inline form-control cursor-pointer w-auto m-0">
                        <div>
                            <input class="form-check-input" type="radio" name="access" value="free"
                                onchange="togglePpvFields(this.value)" {{ old('access', $episode->access ?? 'free') == 'free' ? 'checked' : '' }}>
                    ...
                    <label class="form-check form-check-inline form-control cursor-pointer w-auto m-0">
                        <div>
                            <input class="form-check-input" type="radio" name="access" value="pay-per-view"
                                onchange="togglePpvFields(this.value)" {{ old('access', $episode->access) == 'pay-per-view' ? 'checked' : '' }}>
            ...
            <div class="col-md-4">
                ...
                {{ html()->select('video_upload_type', $upload_url_type->pluck('name', 'name')->prepend(__('placeholder.lbl_select_video_type'), '')->merge(['Embedded' => 'Embedded']), old('video_upload_type', $episode->video_upload_type ?? ''))->class('form-control select2')->id('video_upload_type')->attribute('required') }}
            ...
            <div class="col-md-8">
                <div class="d-none" id="embed_code_input_section">
                    ...
                    {{ html()->textarea('embed_code', old('embed_code', $episode->video_url_input))->class('form-control')->id('embed_code')->placeholder('<iframe ...></iframe>') }}
                </div>

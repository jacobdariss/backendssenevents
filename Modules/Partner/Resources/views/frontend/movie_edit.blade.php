...
@section('title')
    @if($content_type === 'movie') {{ __('partner::partner.edit_movie') }}
    @else {{ __('partner::partner.edit_tvshow') }} @endif
@endsection
...
{{ html()->form('PUT', $content_type === 'movie' ? route('partner.movies.update', $item->id) : route('partner.tvshows.update', $item->id))->attribute('enctype', 'multipart/form-data')->attribute('id', 'form-submit')->open() }}
    ...
    @if(isset($item) && $item->approval_status === 'rejected' && $item->rejection_reason)
    ...
    <div class="card mb-3">
            ...
            <div class="row gy-3">
                ...
                <div class="col-md-4 position-relative">
                    ...
                    <div class="uploaded-image" id="selectedImageContainerThumbnail">
                        @if($item->thumbnail_url)<img src="{{ $item->thumbnail_url }}" class="img-fluid mt-1 box-preview-image">@endif
                    </div>
                    {{ html()->hidden('thumbnail_url')->id('file_url_thumbnail')->value(old('thumbnail_url', $item->thumbnail_url)) }}
                </div>
                ...
                <div class="col-md-6">
                    ...
                    {{ html()->text('name', old('name', $item->name))->class('form-control')->placeholder(__('placeholder.lbl_movie_name'))->attribute('required') }}
                ...
                <div class="col-md-3">
                    ...
                    {{ html()->date('release_date', old('release_date', $item->release_date?->format('Y-m-d')))->class('form-control')->attribute('required') }}
                ...
                <div class="col-md-3">
                    ...
                    {{ html()->text('duration', old('duration', $item->duration))->class('form-control')->placeholder('01:45:00') }}
                </div>
                @endif
                        ...
                        <label class="form-check form-check-inline form-control cursor-pointer w-auto m-0">
                            <div>
                                <input class="form-check-input" type="radio" name="movie_access" value="free"
                                    ...
                                    {{ old('movie_access', $item->movie_access ?? 'free') == 'free' ? 'checked' : '' }}>
                        ...
                        <label class="form-check form-check-inline form-control cursor-pointer w-auto m-0">
                            <div>
                                <input class="form-check-input" type="radio" name="movie_access" value="pay-per-view"
                                    onchange="togglePpvFields(this.value)"
                                    {{ old('movie_access', $item->movie_access) == 'pay-per-view' ? 'checked' : '' }}>
                ...
                {{-- PPV Fields --}}
                <div class="col-12 {{ old('movie_access', $item->movie_access) == 'pay-per-view' ? '' : 'd-none' }}" id="ppv_fields">
                    <div class="row g-3">
                        <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            ...
                            {{ html()->select('purchase_type', ['rental' => __('messages.lbl_rental'), 'onetime' => __('messages.lbl_one_time_purchase')], old('purchase_type', $item->purchase_type ?? 'rental'))->class('form-control select2')->attribute('onchange', 'toggleAccessDurationPartner(this.value)') }}
                        </div>
                        <div class="col-md-3" id="access_duration_wrapper_partner">
                            ...
                            {{ html()->number('access_duration', old('access_duration', $item->access_duration))->class('form-control')->attribute('min', 1)->attribute('placeholder', '7') }}
                        </div>
                        <div class="col-md-3">
                            {{ html()->label(__('messages.lbl_available_for') . ' (jours)', 'available_for')->class('form-label') }}
                            {{ html()->number('available_for', old('available_for', $item->available_for))->class('form-control')->attribute('min', 1)->attribute('placeholder', '30') }}
                        </div>
                        <div class="col-12">
                ...
                <div class="col-md-4">
                    ...
                    {{ html()->select('video_upload_type', $upload_url_type->pluck('name', 'name')->prepend(__('placeholder.lbl_select_video_type'), '')->merge(['Embedded' => 'Embedded']), old('video_upload_type', $item->video_upload_type ?? ''))->class('form-control select2')->id('video_upload_type')->attributes($content_type === 'movie' ? ['required' => true] : []) }}
                ...
                <div class="col-md-8">
                    ...
                    <div class="d-none" id="video_url_input_section">
                        ...
                        {{ html()->text('video_url_input', old('video_url_input', $item->video_url_input))->class('form-control')->id('video_url_input')->placeholder(__('placeholder.video_url_input')) }}
                    </div>
                ...
                <div class="col-md-4">
                    ...
                    {{ html()->select('trailer_url_type', $upload_url_type->pluck('name', 'name')->prepend(__('placeholder.lbl_select_type'), ''), old('trailer_url_type', $item->trailer_url_type ?? ''))->class('form-control select2')->id('trailer_url_type') }}
                </div>
                <div class="col-md-8">
                    <div id="url_input">
                        {{ html()->label(__('movie.lbl_trailer_url'), 'trailer_url')->class('form-label') }}
                        {{ html()->text('trailer_url', old('trailer_url', $item->trailer_url))->class('form-control')->id('trailer_url')->placeholder(__('placeholder.lbl_trailer_url')) }}
                    </div>
        ...
        <div class="card-footer text-end">
            <a href="{{ $content_type === 'movie' ? $content_type === 'movie' ? route('partner.movies') : route('partner.tvshows') : route('partner.tvshows') }}" class="btn btn-secondary me-2">{{ __('messages.cancel') }}</a>

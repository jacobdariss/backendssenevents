<div class="d-flex gap-2 align-items-center justify-content-end">
    ...
    data-success-message="{{trans('messages.restore_form', ['form' => 'Banner'])}}" href="{{ route('backend.banners.restore', $data->id) }}" data-bs-toggle="tooltip" title="{{ __('messages.restore') }}">
        ...
        <!-- Force Delete link -->
        <a href="{{ route('backend.banners.force_delete', $data->id) }}" id="delete-locations-{{ $data->id }}" class="btn btn-danger-subtle btn-sm fs-4" data-type="ajax" data-method="DELETE" data-token="{{ csrf_token() }}" data-bs-toggle="tooltip" title="{{ __('messages.force_delete') }}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_entity', ['name' => $data->title ?? $data->id, 'entity' => 'Banner']) }}">
            <i class="ph ph-trash align-middle"></i>
        </a>
        @endhasPermission
    @endif

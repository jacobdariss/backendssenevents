<div class="d-flex gap-2 align-items-center justify-content-end">
  ...
  {{-- @hasPermission('force_delete_faqs') --}}
      <!-- Force Delete Button -->
      <a href="{{ route('backend.faqs.force_delete', $data->id) }}" id="force-delete-faq-{{ $data->id }}" class="btn btn-danger-subtle btn-sm fs-4" data-type="ajax" data-method="DELETE" data-token="{{ csrf_token() }}" data-bs-toggle="tooltip" title="{{ __('messages.force_delete') }}" data-confirm="{{ __('messages.are_you_sure_permanently_delete_faq', ['name' => $data->question]) }}">
          <i class="ph ph-trash align-middle"></i>
      </a>
  {{-- @endhasPermission --}}
  @endif

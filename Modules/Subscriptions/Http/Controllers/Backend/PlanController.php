...
class PlanController extends Controller
{
    ...
    public function index_data(Datatables $datatable, Request $request)
    {
        ...
        $datatable = $datatable->eloquent($query)
            ...
            ->filterColumn('level', function($query, $keyword) {
                // UI renders "Level X" but DB stores numeric X, so normalize search.
                $raw = trim((string) $keyword);
                $num = preg_replace('/[^0-9]/', '', $raw);

                // If user typed "Level 3" -> search level = 3
                // If user typed "3" -> search level like 3 (as string) to work with partial matches
                if ($num !== '') {
                    $query->whereRaw("CAST(level AS CHAR) LIKE ?", ['%' . $num . '%']);
                    return;
                }

                // Fallback: if they typed just "level", don't filter anything extra.
                // Otherwise try a generic cast match (rare)
                $query->whereRaw("CAST(level AS CHAR) LIKE ?", ['%' . $raw . '%']);
            })
        ...
        // Handle global search for "Level 3" vs "Level3" vs "3" (and plain "level")
        $datatable->filter(function ($query) use ($request) {
            ...
            $normalized = strtolower(preg_replace('/\s+/', '', $search)); // "Level 3" -> "level3"

            // If the user typed only "level", match all levels via "levelX"
            if ($normalized === 'level') {
                $query->orWhereRaw("LOWER(CONCAT('level', level)) LIKE ?", ['%level%']);
                return;
            }

            if (preg_match('/^level?\d+$/', $normalized) || is_numeric($normalized)) {
                $query->orWhereRaw("CAST(level AS CHAR) LIKE ?", ['%' . $normalized . '%'])
                      ->orWhereRaw("LOWER(CONCAT('level', level)) LIKE ?", ['%' . $normalized . '%']);
            }
        }, true);
    ...
    public function edit(int $id)
    {
        ...
        $planLimits = PlanLimitationMapping::where('plan_id', $id)
            ->whereHas('limitation_data', function($query) {

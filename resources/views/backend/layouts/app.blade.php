...
{{ !isset($isBanner) ? 'iq-banner' : '' }} default">
    ...
    <script>
        ...
        function notificationList(type = '') {
            ...
            $container.html('<div class="p-3 text-center text-muted"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
            $.ajax({
                type: 'get',
                url: url,
                data: { 'type': type },
                ...
                success: function(res) {
                    ...
                    if (res && res.type == "markas_read") {
                        notificationList();
                    }
                    $('.notify_count').removeClass('notification_tag').text('');
                },
                error: function() {
                    $container.html('<div class="p-3 text-center text-muted small">Unable to load notifications.</div>');
                }
            });
        }
